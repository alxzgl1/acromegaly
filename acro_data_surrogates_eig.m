...
% Function 
%-------------------------------------------------------------------------------
function acro_data_surrogates_eig()
...
SY = zeros(nFeatures, nSurrogates);
for iSurrogate = 1:nSurrogates
  fprintf(1, '%d / %d\n', iSurrogate, nSurrogates);
  v = Y(:, :, iSurrogate);
...
yMin = min([sx(:); SY(:)]);
yMax = max([sx(:); SY(:)]);
subplot(2, 3, 3); plot(sx); ylim([yMin, yMax]); xlim([1, 100]); box off;
xlabel('metabolites'); ylabel('eigen values'); title('C1 | Eigen values of Correlation matrix B1', 'FontWeight', 'normal');
for iSurrogate = 1:nSurrogates
  subplot(2, 3, 6); plot(SY(:, iSurrogate)); hold on; 
end
ylim([yMin, yMax]); xlim([1, 100]); box off;
xlabel('metabolites'); ylabel('eigen values'); title('C2 | Eigen values of Correlation matrix B2 (5 runs)', 'FontWeight', 'normal');
% save figure
aFile = sprintf('%s_MV%d%d', aFile, round(100 * nMaxMissingValuesBySubjects), round(100 * nMaxMissingValuesByFeatures));
aFilename = [aPath, '\\', '_analysis', '\\', 'eig', '\\', 'surrogates_', aSurrogates, '\\', aFile, '.png'];
