...
% Function 
%-------------------------------------------------------------------------------
function acro_PCA_and_classification_GMLVQ()
...
for bShuffle = 0:1
  ...
  for iRandomSample = 1:nRandomSamples
    ...
    if strcmp(aClassifier, 'GMLVQ')
      % GMLVQ - Generalised Matrix LVQ
      GMLVQ_model = GMLVQ_train(X_TR', (y_TR + 1)', 'Display', 'off');

      % subplot(3, 3, 1); bar(GMLVQ_model.w(1, :));
      % subplot(3, 3, 4); bar(GMLVQ_model.w(2, :));
      % subplot(3, 3, 7); bar(diag(GMLVQ_model.omega' * GMLVQ_model.omega));

      % GRLVQ - Generalised Relevance LVQ
      % GRLVQ_model = GRLVQ_train(X_TR', (y_TR + 1)', 'Display', 'off');

      % subplot(3, 3, 2); bar(GRLVQ_model.w(1, :));
      % subplot(3, 3, 5); bar(GRLVQ_model.w(2, :));
      % subplot(3, 3, 8); bar(GRLVQ_model.lambda);

      GMLVQ_labels = GMLVQ_classify(X_TS', GMLVQ_model);
